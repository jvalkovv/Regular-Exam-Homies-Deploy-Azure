...
resource "azurerm_linux_web_app" "lwa" {
  name                = "${var.azurerm_service_app_name}-${random_integer.ri.result}"
...
resource "azurerm_mssql_server" "mssql" {
  name                         = "${var.azurerm_sql_server_name}-${random_integer.ri.result}"
  location                     = var.resource_group_location
  resource_group_name          = "${var.resource_group_name}-${random_integer.ri.result}"
  version                      = "12.0"
...
resource "azurerm_mssql_database" "mssqldb" {
  name           = "${var.azurerm_sql_database_name}-${random_integer.ri.result}"
...
resource "azurerm_mssql_firewall_rule" "mfr" {
  name             = "${var.azurerm_mssql_firewall_rule}-${random_integer.ri.result}"
  server_id        = azurerm_mssql_server.mssql.id
